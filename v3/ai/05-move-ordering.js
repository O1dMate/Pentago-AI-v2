...
function GetDescription() {
	return {
		name: '05 - Move Ordering (Prioritize 4 in a Row)',
		des: '',
	};
}
...
function GetEmptyIndices(game, currentTurn) {
	...
	if (tempScore < -500000) {
		...
		for (let i = 0; i < game.length; ++i) {
			if (game[i] === PIECES.EMPTY) {
				...
				for (let quadrant = 0; quadrant <= 3; ++quadrant) {
					...
					else worryingMoves.push([i, quadrant, false, (((i << 2) | (quadrant)) << 1) | 0, tempScore]);
					...
					else worryingMoves.push([i, quadrant, true, (((i << 2) | (quadrant)) << 1) | 1, tempScore]);
				}
		...
		if (priorityMoves.length > 0) worryingMoves = [];
	} else {
		for (let i = 0; i < game.length; ++i) {
			if (game[i] === PIECES.EMPTY) {
				// Place the marble
				...
				for (let quadrant = 0; quadrant <= 3; ++quadrant) {
					...
					else if (tempScore < -500) worryingMoves.push([i, quadrant, false, (((i << 2) | (quadrant)) << 1) | 0, tempScore]);
					...
					else if (tempScore < -500) worryingMoves.push([i, quadrant, true, (((i << 2) | (quadrant)) << 1) | 1, tempScore]);
	...
	} else if (worryingMoves.length > 0) {
		worryingMoves.sort((a, b) => {
			// Sort according to the 4th element (the score)
			return a[4] > b[4] ? -1 : 1;
		});
	}
	...
	return [...priorityMoves, ...otherMoves, ...worryingMoves];
}
