...
function GetEmptyIndices(game, targetColor) {
	...
	if (validMoves.length === 0) {
		for (let i = 0; i < game.length; ++i) {
			if (game[i] !== PIECES.EMPTY) continue;
			validMoves.push([i, 0, false]); validMoves.push([i, 0, true]);
			validMoves.push([i, 1, false]); validMoves.push([i, 1, true]);
			validMoves.push([i, 2, false]); validMoves.push([i, 2, true]);
			validMoves.push([i, 3, false]); validMoves.push([i, 3, true]);
		}
	}
	...
	validMoves.sort((a,b) => {
		...
		// return scoreLookup.get((1000 * a[0]) + a[1] + (a[2] ? 4 : 0)) > scoreLookup.get((1000 * b[0]) + b[1] + (b[2] ? 4 : 0)) ? -1 : 1;
	});
	...
	// let iter = scoreLookup.entries();
	
	// let curValue = iter.next().value;

	// while (curValue) {
	// 	scoreList.push(curValue);
	// 	curValue = iter.next().value;
	// }

	// scoreList.sort((a,b) => a[1] > b[1] ? -1 : 1);

	// scoreList.forEach(a => console.log(a));
